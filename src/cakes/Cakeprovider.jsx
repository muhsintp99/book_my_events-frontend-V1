...
const [deliveryTime, setDeliveryTime] = useState({
  ...
  if (!deliveryTime.minDays || !deliveryTime.maxDays) {
    errors.push("Estimated delivery time is required for Cake vendors");
  }
}
...
if (isCakeModule()) {
  ...
  payload.append("maxDeliveryDays", deliveryTime.maxDays);
  payload.append("deliveryUnit", deliveryTime.unit);
}
...
const isCakeModule = () => {
  const mod = allModules.find((m) => m._id === formData.module);
  if (!mod) return false;
  return mod.title.toLowerCase().includes("cake");
};
  ...
  >
    ...
    <Box sx={{ display: "flex", gap: 2, mb: 2 }}>
      <TextField
        ...
        type="number"
        label="Minimum Time"
        value={deliveryTime.minDays}
        onChange={(e) =>
          setDeliveryTime({ ...deliveryTime, minDays: e.target.value })
        }
      />
      ...
      <TextField
        ...
        type="number"
        label="Maximum Time"
        value={deliveryTime.maxDays}
        onChange={(e) =>
          setDeliveryTime({ ...deliveryTime, maxDays: e.target.value })
        }
      />
    </Box>
