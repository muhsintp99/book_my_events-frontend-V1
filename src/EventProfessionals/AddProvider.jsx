...
function AddProfessional() {
  ...
  const fetchModules = async () => {
    try {
      ...
      if (activeModuleId && filtered.length) {
        ...
        const profMod = combined.find(
          (m) => (m.title || '').toLowerCase().includes('professional')
        );
        if (profMod) {
          setFormData((prev) => ({ ...prev, module: profMod._id }));
          setModules([profMod]);
        }
      }
    } catch (err) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Event Professionals specific fields
      ...
      if (isEditMode) {
        ...
        setTimeout(() => {
          navigate('/eventprofessionals/professionalprovider');
        }, 2000);
        return;
      }
  ...
  const handleReset = () => {
    // Find Event Professionals module ID from allModules
    const profMod = (allModules || []).find(
      (m) => (m.title || '').toLowerCase().includes('professional')
    );
    ...
    setFormData({
      ...
      maximumDeliveryTime: '',
      zone: '',
      module: profMod ? profMod._id : activeModuleId || '',
      latitude: '',
      longitude: '',
      ownerFirstName: '',
      ownerLastName: '',
  ...
  return (
    <Box sx={{ p: 3, backgroundColor: '#f9f9f9', borderRadius: 2 }}>
      <Typography variant="h4" fontWeight="bold" sx={{ mb: 3 }}>
        {isEditMode ? 'Edit Event Professionals Provider' : 'Add Event Professionals Provider'}
      </Typography>
      ...
      <Box sx={{ mb: 3, p: 2, border: '1px solid #e0e0e0', borderRadius: 2, bgcolor: '#fff' }}>
        <Typography variant="h5" fontWeight="bold" sx={{ mb: 2 }}>
          Event Professional Specialisation & Services
        </Typography>
