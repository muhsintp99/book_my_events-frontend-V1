...
const EventProfessionalCoupon = () => {
    ...
    const fetchCoupons = async (searchQuery = "") => {
        setLoading(true);
        try {
            const url = `${API_BASE_URL}/event-professional-coupons?search=${encodeURIComponent(searchQuery)}`;
    ...
    const handleSave = async () => {
        setLoading(true);
        try {
            const response = await fetch(`${API_BASE_URL}/event-professional-coupons`, {
                method: "POST",
    ...
    return (
        <Box p={3}>
            <Typography variant="h4" fontWeight="bold" gutterBottom>Event Professional Coupons</Typography>
            ...
            <Stack direction="row" justifyContent="space-between" mb={3}>
                ...
                <TextField size="small" placeholder="Search by code" value={search} onChange={(e) => setSearchTerm(e.target.value)} />
            </Stack>
...
export default EventProfessionalCoupon;
