...
export default function CategoryManagement() {
  ...
  const fetchModules = useCallback(async () => {
    ...
    try {
      ...
      // Find Event Professionals module as default
      const profModule = normalizedModules.find(m =>
        (m.title || m.name || '').toLowerCase().includes('professional')
      );

      let defaultModuleId = '';

      if (profModule) {
        defaultModuleId = profModule._id;
  ...
  const handleReset = () => {
    const profModule = modules.find(m =>
      (m.title || m.name || '').toLowerCase().includes('professional')
    );

    const defaultModuleId =
      profModule?._id || (modules.length > 0 ? modules[0]._id : '');
