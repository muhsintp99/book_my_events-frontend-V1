...
import {
  ...
  Refresh, CheckCircle, Groups,
  WorkspacePremium, BusinessCenter
...
const Professionallist = () => {
  ...
  // ✅ FIXED: correct API endpoint is /event-professional
  ...
  const fetchPackages = async () => {
    try {
      ...
      console.log('EVENT PROFESSIONAL VENDORS API:', vendorData);
          ...
          try {
            ...
            pkgList.forEach((pkg, idx) => {
              allMapped.push({
                _id: pkg._id,
                id: `${vIndex}-${idx}`,
                title: pkg.packageName || 'Professional Service',
                description: pkg.description || '',
                price: pkg.packagePrice ?? 0,
                advance: pkg.advanceBookingAmount ?? 0,
                category: pkg.category || 'Expert',
    ...
    <Box sx={{ p: 4, bgcolor: themeColors.background, minHeight: '100vh' }}>
      ...
      <Box sx={{ mb: 4, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Box>
          ...
          <Typography variant="subtitle1" sx={{ color: themeColors.textSecondary, fontWeight: 600, mt: 0.5, fontSize: '0.88rem' }}>
        ...
        <Button
          ...
          sx={{
            ...
            color: themeColors.textSecondary, borderColor: themeColors.border
          }}
        >
          Reload
        </Button>
      ...
      <Paper elevation={0} sx={{
        ...
        >
          {Object.values(expandedVendors).some(v => v) ? 'Hide Professionals' : 'View All Professionals'}
        </Button>
      </Paper>
      ...
      {!loading && vendorGroups.length === 0 && (
        <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', py: 10, gap: 2 }}>
          <BusinessCenter sx={{ fontSize: 52, color: themeColors.border }} />
          <Typography sx={{ color: themeColors.textSecondary, fontWeight: 700, fontSize: '1rem' }}>
            No professional packages found.
          </Typography>
        ...
        {vendorGroups.map((vendor) => {
          ...
          return (
            ...
            >
              ...
              >
                <Avatar sx={{
                  width: 48, height: 48, fontSize: '1.1rem', fontWeight: 900,
                ...
                <Box sx={{ flex: 1 }}>
                  <Typography sx={{ fontWeight: 900, fontSize: '1.1rem', color: themeColors.textMain }}>
                    {vendor.providerName}
                  </Typography>
                ...
                <Box sx={{ textAlign: 'right', px: 1.5 }}>
                  <Typography sx={{ fontWeight: 900, color: themeColors.textSecondary, fontSize: '0.75rem' }}>
                    {vendor.packages.length} SPECIALTIES
                  </Typography>
                  <Chip
                    ...
                    <Table size="medium">
                      <TableHead>
                        <TableRow sx={{ '& th': { borderBottom: `2px solid ${themeColors.border}`, py: 1.2 } }}>
                          <TableCell sx={{ fontWeight: 900, color: themeColors.textSecondary, fontSize: '0.78rem' }}>EXPERT ROLE</TableCell>
                          <TableCell sx={{ fontWeight: 900, color: themeColors.textSecondary, fontSize: '0.78rem' }}>BASE FEE (INR)</TableCell>
                          <TableCell sx={{ fontWeight: 900, color: themeColors.textSecondary, fontSize: '0.78rem' }} align="center">VIP STATUS</TableCell>
                          <TableCell sx={{ fontWeight: 900, color: themeColors.textSecondary, fontSize: '0.78rem' }} align="center">LISTING</TableCell>
                          <TableCell sx={{ fontWeight: 900, color: themeColors.textSecondary, fontSize: '0.78rem' }} align="right">OPTIONS</TableCell>
                        </TableRow>
                      </TableHead>
                      <TableBody>
                        {vendor.packages.map((pkg) => (
                          ...
                          >
                            <TableCell>
                              ...
                              <Chip
                                label={pkg.category || 'Expert'}
                                size="small"
                                sx={{ height: 18, fontSize: '0.65rem', fontWeight: 900, mt: 0.5, bgcolor: '#f1f5f9', color: themeColors.textSecondary }}
                              />
                            </TableCell>
                            <TableCell>
        ...
        <DialogContent sx={{ pb: 4 }}>
          {selectedPackage && (
            <Box>
              ...
              <Typography variant="body2" sx={{ color: themeColors.textSecondary, mb: 3, lineHeight: 1.6, fontSize: '0.9rem' }}>
                {selectedPackage.description || 'Highly experienced professional with a track record of executing flawless events with precision.'}
              </Typography>
              <Grid container spacing={2}>
                <Grid item xs={6}>
                  <Paper variant="outlined" sx={{ p: 2, borderRadius: '14px', bgcolor: themeColors.background }}>
                    <Typography variant="caption" sx={{ fontWeight: 800, color: themeColors.textSecondary }}>FEE</Typography>
                    <Typography sx={{ fontWeight: 900, color: themeColors.success, fontSize: '1.2rem' }}>
                ...
                <Grid item xs={6}>
                  <Paper variant="outlined" sx={{ p: 2, borderRadius: '14px', bgcolor: themeColors.background }}>
                    ...
                    <Typography sx={{ fontWeight: 900, color: themeColors.textMain, fontSize: '1.2rem' }}>
              ...
              <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
                {['Leadership', 'Event Planning', 'Execution', 'Coordination'].map(c => (
                  <Chip key={c} label={c} size="small" variant="outlined" sx={{ fontWeight: 700 }} />
                ))}
              </Stack>
            </Box>
          )}
      ...
      >
        <Alert severity={notification.severity} variant="filled" sx={{ borderRadius: '18px', fontWeight: 700 }}>
