...
const MakeupList = () => {
  ...
  const toINR = (aed) => Math.round((aed || 0) * 22.8);
    ...
    try {
      ...
      if (data?.data && Array.isArray(data.data)) {
        const mapped = data.data.map((m, idx) => ({
          ...
          trialIncluded: m.trialMakeupIncluded ?? false,
          travelToVenue: m.travelToVenue ?? false,
        ...
        setAllMakeups(mapped); setMakeups(mapped);
        
        // Expand All by default for maximum visibility
  ...
  const handleStatusUpdate = async (_id, type) => {
    ...
    setMakeups(p => p.map(c => c._id === _id ? { ...c, [valKey]: !c[valKey] } : c));
    try {
  ...
  const handleView = (pkg) => { setSelectedMakeup(pkg.rawMakeup); setOpenViewDialog(true); };

  const stats = {
    total: allMakeups.length,
    active: allMakeups.filter(c => c.isActive).length,
    topPick: allMakeups.filter(c => c.isTopPick).length,
    vendors: [...new Set(allMakeups.map(c => c.providerId))].length,
  };
    ...
    <Box sx={{ p: 4, minHeight: '100vh', bgcolor: themeColors.background, color: themeColors.textMain }}>
      ...
      <Box sx={{ mb: 4, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Box>
          ...
          <Typography variant="subtitle1" sx={{ color: themeColors.textSecondary, fontWeight: 600, mt: 0.2, fontSize: '0.88rem' }}>
            Complete vendor & package overview.
          </Typography>
        </Box>
        <Box sx={{ display: 'flex', gap: 1.5 }}>
          <Button variant="outlined" startIcon={<Refresh fontSize="small" />} onClick={() => fetchMakeups()} sx={{ borderRadius: '12px', textTransform: 'none', fontWeight: 700, px: 2.2, height: 42, fontSize: '0.82rem', borderColor: themeColors.border, color: themeColors.textSecondary }}>Refresh</Button>
          <Button variant="contained" disableElevation onClick={() => fetchMakeups(true)} sx={{ borderRadius: '12px', textTransform: 'none', fontWeight: 700, px: 2.5, height: 42, fontSize: '0.82rem', background: themeColors.gradientPrimary, '&:hover': { opacity: 0.9 } }}>Top Picks</Button>
        </Box>
      </Box>

      {/* Red Gradient Stats */}
      <Box sx={{ display: 'flex', gap: 2.5, mb: 4, flexWrap: 'wrap' }}>
        <HeaderStat label="Packages" value={stats.total} icon={<ViewList />} color={themeColors.accent} gradient={themeColors.gradientPrimary} />
        ...
        <HeaderStat label="Elite Vendors" value={stats.vendors} icon={<Storefront />} color={themeColors.primary} />
      </Box>
      ...
      <Paper elevation={0} sx={{ p: 1.8, mb: 3.5, borderRadius: '18px', border: '1px solid', borderColor: themeColors.border, display: 'flex', alignItems: 'center', justifyContent: 'space-between', bgcolor: 'white' }}>
        <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
          <TextField
            placeholder="Search providers or packages..."
            size="small"
            value={searchTerm}
        ...
        >
          {Object.values(expandedVendors).some(v => v) ? 'Collapse Data' : 'View Full List'}
        </Button>
      </Paper>

      {/* Vendor Groups (Expanded Red Cards) */}
              ...
              <Box onClick={() => toggleVendor(vendor.providerId)} sx={{
                ...
                <Box sx={{ flex: 1 }}>
                  ...
                  <Typography variant="body1" sx={{ color: themeColors.textSecondary, fontWeight: 600, fontSize: '0.82rem' }}>
                    ...
                    {vendor.packages?.[0]?.rawMakeup?.provider?.zone?.name && ` • Available in ${vendor.packages[0].rawMakeup.provider.zone.name}`}
                  </Typography>
                </Box>
                    ...
                    <Table size="medium">
                      <TableHead>
                        <TableRow sx={{ '& th': { borderBottom: '2px solid' + themeColors.border, py: 1.2 } }}>
                          <TableCell sx={{ fontWeight: 900, color: themeColors.textSecondary, fontSize: '0.78rem' }}>PACKAGE INFO</TableCell>
                          ...
                          <TableRow key={pkg._id} sx={{ '& td': { py: 1.6 }, '&:hover': { bgcolor: '#F9FAFB' } }}>
                            <TableCell>
                              ...
                              <Box sx={{ display: 'flex', gap: 1, mt: 0.5 }}>
                                {pkg.trialIncluded && <Chip label="Trial Included" size="small" icon={<DoneAll sx={{ fontSize: '10px !important' }} />} sx={{ height: 18, fontSize: '0.6rem', fontWeight: 700, bgcolor: '#f0fdf4', color: '#16a34a' }} />}
                                {pkg.travelToVenue && <Chip label="Can Travel" size="small" icon={<FlightTakeoff sx={{ fontSize: '10px !important' }} />} sx={{ height: 18, fontSize: '0.6rem', fontWeight: 700, bgcolor: '#eff6ff', color: '#2563eb' }} />}
                              </Box>
                            </TableCell>
                            <TableCell>
                              <Typography sx={{ fontWeight: 900, color: themeColors.success, fontSize: '1.05rem' }}>₹{toINR(pkg.finalPrice).toLocaleString()}</Typography>
                              {pkg.basePrice > pkg.finalPrice && <Typography sx={{ color: themeColors.textSecondary, textDecoration: 'line-through', fontSize: '0.75rem' }}>₹{toINR(pkg.basePrice).toLocaleString()}</Typography>}
                            </TableCell>
      ...
      <Dialog open={openViewDialog} onClose={() => setOpenViewDialog(false)} maxWidth="sm" fullWidth PaperProps={{ sx: { borderRadius: '24px' } }}>
        ...
        <DialogContent sx={{ pb: 4 }}>
          {selectedMakeup && (
            <Box>
              <Typography variant="h5" sx={{ fontWeight: 900, mb: 1.5, color: themeColors.accent }}>{selectedMakeup.packageTitle}</Typography>
              <Typography sx={{ color: themeColors.textSecondary, mb: 3, lineHeight: 1.6, fontSize: '0.9rem' }}>{selectedMakeup.description}</Typography>
              ...
              <Grid container spacing={2}>
                <Grid item xs={6}>
                  <Paper variant="outlined" sx={{ p: 2, borderRadius: '16px', bgcolor: '#F9FAFB' }}>
                    ...
                    <Typography variant="h5" sx={{ fontWeight: 900, color: themeColors.success }}>₹{toINR(selectedMakeup.finalPrice).toLocaleString()}</Typography>
                  </Paper>
                </Grid>
                <Grid item xs={6}>
                  <Paper variant="outlined" sx={{ p: 2, borderRadius: '16px', bgcolor: '#F9FAFB' }}>
                    <Typography variant="caption" sx={{ fontWeight: 800, color: themeColors.textSecondary }}>STATUS</Typography>
                    <Typography variant="h6" sx={{ fontWeight: 900, color: selectedMakeup.isActive ? themeColors.success : themeColors.danger }}>
                      {selectedMakeup.isActive ? 'ACTIVE' : 'INACTIVE'}
                    </Typography>
                  </Paper>
                </Grid>
              </Grid>
              ...
              <Typography variant="subtitle2" sx={{ fontWeight: 900, mb: 2 }}>Core Inclusions:</Typography>
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                <Chip label="Professional Makeup" size="small" sx={{ fontWeight: 700 }} />
                {selectedMakeup.trialMakeupIncluded && <Chip label="Complimentary Trial" color="success" size="small" sx={{ fontWeight: 700 }} />}
                {selectedMakeup.travelToVenue && <Chip label="On-Site Service" color="primary" size="small" sx={{ fontWeight: 700 }} />}
              </Box>
            </Box>
          )}
        </DialogContent>
