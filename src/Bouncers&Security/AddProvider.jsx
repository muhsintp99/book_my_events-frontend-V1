...
function AddBouncer() {
  ...
  const fetchModules = async () => {
    try {
      ...
      if (activeModuleId && filtered.length) {
        ...
        const bouncerMod = combined.find(
          (m) => (m.title || '').toLowerCase().includes('bouncer') || (m.title || '').toLowerCase().includes('security')
        );
        if (bouncerMod) {
          setFormData((prev) => ({ ...prev, module: bouncerMod._id }));
          setModules([bouncerMod]);
        }
      }
    } catch (err) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Bouncers & Security specific fields
      ...
      if (isEditMode) {
        ...
        setTimeout(() => {
          navigate('/bouncers/bouncerprovider');
        }, 2000);
        return;
      }
  ...
  const handleReset = () => {
    // Find Bouncers & Security module ID from allModules
    const bouncerMod = (allModules || []).find(
      (m) => (m.title || '').toLowerCase().includes('bouncer') || (m.title || '').toLowerCase().includes('security')
    );
    ...
    setFormData({
      ...
      maximumDeliveryTime: '',
      zone: '',
      module: bouncerMod ? bouncerMod._id : activeModuleId || '',
      latitude: '',
      longitude: '',
      ownerFirstName: '',
      ownerLastName: '',
  ...
  return (
    <Box sx={{ p: 3, backgroundColor: '#f9f9f9', borderRadius: 2 }}>
      <Typography variant="h4" fontWeight="bold" sx={{ mb: 3 }}>
        {isEditMode ? 'Edit Bouncers & Security Provider' : 'Add Bouncers & Security Provider'}
      </Typography>
      ...
      <Box sx={{ mb: 3, p: 2, border: '1px solid #e0e0e0', borderRadius: 2, bgcolor: '#fff' }}>
        <Typography variant="h5" fontWeight="bold" sx={{ mb: 2 }}>
          Bouncers & Security Specialisation & Services
        </Typography>
