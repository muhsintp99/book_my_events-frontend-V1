...
export default function CategoryManagement() {
  ...
  const fetchModules = useCallback(async () => {
    ...
    try {
      ...
      // Find Bouncers & Security module as default
      const bouncerModule = normalizedModules.find(m =>
        (m.title || m.name || '').toLowerCase().includes('bouncer') ||
        (m.title || m.name || '').toLowerCase().includes('security')
      );

      let defaultModuleId = '';

      if (bouncerModule) {
        defaultModuleId = bouncerModule._id;
  ...
  const handleReset = () => {
    const bouncerModule = modules.find(m =>
      (m.title || m.name || '').toLowerCase().includes('bouncer') ||
      (m.title || m.name || '').toLowerCase().includes('security')
    );

    const defaultModuleId =
      bouncerModule?._id || (modules.length > 0 ? modules[0]._id : '');
