...
const Bouncerlist = () => {
  ...
  const API_URL = `${API_BASE_URL}/bouncer`;
    ...
    <Box sx={{ p: 4, minHeight: '100vh', bgcolor: themeColors.background, color: themeColors.textMain }}>
      ...
      <Box sx={{ mb: 4, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <Box sx={{ display: 'flex', gap: 1.5 }}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, bgcolor: 'white', px: 2, borderRadius: '12px', border: '1px solid', borderColor: themeColors.border }}>
            <Typography sx={{ fontWeight: 900, color: themeColors.textSecondary, fontSize: '0.75rem' }}>Top Pick</Typography>
            <Switch size="small" checked={packages.some(p => p.isTopPick)} sx={{ '& .MuiSwitch-switchBase.Mui-checked': { color: themeColors.accent } }} />
          </Box>
        </Box>
      </Box>
      ...
      <Box sx={{ display: 'flex', gap: 2.5, mb: 4, flexWrap: 'wrap' }}>
        <HeaderStat label="Guard Packages" value={stats.total} icon={<Security />} color={themeColors.accent} gradient={themeColors.gradientPrimary} />
        <HeaderStat label="Active" value={stats.active} icon={<VerifiedUser />} color={themeColors.success} gradient={themeColors.gradientSuccess} />
        <HeaderStat label="Top Rated" value={stats.topPick} icon={<Star />} color={themeColors.warning} />
        <HeaderStat label="Agencies" value={stats.vendors} icon={<Storefront />} color={themeColors.primary} />
      </Box>
      ...
      <Paper elevation={0} sx={{ p: 1.8, mb: 3.5, borderRadius: '18px', border: '1px solid', borderColor: themeColors.border, display: 'flex', alignItems: 'center', justifyContent: 'space-between', bgcolor: 'white' }}>
        <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
          <TextField
            placeholder="Search security group or bouncers..."
            size="small"
            value={searchTerm}
        ...
        >
          {Object.values(expandedVendors).some(v => v) ? 'Hide Agencies' : 'View All Agencies'}
        </Button>
      </Paper>
            ...
            <Paper key={vendor.providerId} elevation={0} sx={{
              ...
              <Box onClick={() => toggleVendor(vendor.providerId)} sx={{
                ...
                <Box sx={{ textAlign: 'right', px: 1.5 }}>
                  ...
                  <Chip label={`${activeCount} READY`} size="small" sx={{ fontWeight: 900, bgcolor: themeColors.success, color: 'white', mt: 0.4, height: 22, fontSize: '0.65rem' }} />
                </Box>
              </Box>
                    ...
                    <Table size="medium">
                      <TableHead>
                        <TableRow sx={{ '& th': { borderBottom: '2px solid' + themeColors.border, py: 1.2 } }}>
                          <TableCell sx={{ fontWeight: 900, color: themeColors.textSecondary, fontSize: '0.78rem' }}>GUARD TYPE</TableCell>
                          <TableCell sx={{ fontWeight: 900, color: themeColors.textSecondary, fontSize: '0.78rem' }}>BASE RATE (INR)</TableCell>
                          ...
                          <TableRow key={pkg._id} sx={{ '& td': { py: 1.6 }, '&:hover': { bgcolor: '#F9FAFB' } }}>
                            <TableCell>
                              ...
                              <Box sx={{ display: 'flex', gap: 1, mt: 0.5 }}>
                                <Chip label="Trained" size="small" icon={<Shield sx={{ fontSize: '10px !important' }} />} sx={{ height: 18, fontSize: '0.6rem', fontWeight: 700, bgcolor: '#eff6ff', color: '#1d4ed8' }} />
                                <Chip label="VIP Security" size="small" sx={{ height: 18, fontSize: '0.6rem', fontWeight: 700, bgcolor: '#fdf2f8', color: '#be185d' }} />
                              </Box>
                            </TableCell>
                            <TableCell>
                              <Typography sx={{ fontWeight: 900, color: themeColors.success, fontSize: '1.05rem' }}>₹{pkg.price.toLocaleString()}</Typography>
                              <Typography variant="caption" sx={{ color: themeColors.textSecondary, fontSize: '0.65rem' }}>Advance req: ₹{pkg.advance.toLocaleString()}</Typography>
                            </TableCell>
                            ...
                            <TableCell align="right">
                              <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 0.6 }}>
                                <Tooltip title="View Verification Details"><IconButton size="small" onClick={() => handleView(pkg)} sx={{ color: themeColors.accent }}><VisibilityOutlined sx={{ fontSize: 18 }} /></IconButton></Tooltip>
      ...
      <Dialog open={openViewDialog} onClose={() => setOpenViewDialog(false)} maxWidth="sm" fullWidth PaperProps={{ sx: { borderRadius: '24px' } }}>
        <DialogTitle sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', pt: 3 }}>
          <Typography sx={{ fontWeight: 900, fontSize: '1.2rem' }}>Package Specifics</Typography>
        ...
        <DialogContent sx={{ pb: 4 }}>
          {selectedPackage && (
            <Box>
              ...
              <Typography sx={{ color: themeColors.textSecondary, mb: 3, lineHeight: 1.6, fontSize: '0.9rem' }}>{selectedPackage.description || 'Professional security and crowd control services tailored for your grand event.'}</Typography>
              
              <Grid container spacing={2}>
                <Grid item xs={6}>
                  <Paper variant="outlined" sx={{ p: 2, borderRadius: '16px', bgcolor: '#F9FAFB' }}>
                    <Typography variant="caption" sx={{ fontWeight: 800, color: themeColors.textSecondary }}>HIRE RATE</Typography>
              ...
              <Typography variant="subtitle2" sx={{ fontWeight: 900, mb: 2 }}>Security Features:</Typography>
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                <Chip label="Emergency Response" size="small" variant="outlined" sx={{ fontWeight: 700 }} />
                <Chip label="Crowd Management" size="small" variant="outlined" sx={{ fontWeight: 700 }} />
                <Chip label="Premium Attire" size="small" variant="outlined" sx={{ fontWeight: 700 }} />
              </Box>
            </Box>
          )}
        </DialogContent>
