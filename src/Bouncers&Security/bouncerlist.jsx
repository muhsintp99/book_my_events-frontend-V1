...
const HeaderStat = ({ label, value, icon, color, gradient }) => (
  ...
  >
    ...
    <Box>
      <Typography
        variant="caption"
        sx={{ color: themeColors.textSecondary, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.8px', fontSize: '0.7rem' }}
      >
        {label}
      </Typography>
...
const Bouncerlist = () => {
  ...
  const MODULE_ID = localStorage.getItem('moduleDbId'); // ✅ AUTO USE YOUR STORED ID
  ...
  try {
    ...
    console.log("BOUNCER VENDORS API:", vendorData);
    ...
    // Step 2: For each vendor, fetch their packages
    const allMapped = [];
    const expanded = {};
  ...
  const filtered = packages.filter((c) => `${c.title} ${c.providerName}`.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  const handleView = (pkg) => {
    setSelectedPackage(pkg.rawPackage);
  ...
  return (
    <Box sx={{ p: 4, minHeight: '100vh', bgcolor: themeColors.background, color: themeColors.textMain }}>
      ...
      <Box sx={{ mb: 4, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <Box sx={{ display: 'flex', gap: 1.5 }}>
          ...
          >
            ...
            <Switch
              size="small"
              checked={packages.some((p) => p.isTopPick)}
              sx={{ '& .MuiSwitch-switchBase.Mui-checked': { color: themeColors.accent } }}
            />
          </Box>
        </Box>
      </Box>
        ...
        <Button
          ...
          onClick={() => {
            ...
            if (anyVisible) setExpandedVendors({});
              ...
              const all = {};
              [...new Set(filtered.map((f) => f.providerId))].forEach((id) => {
                all[id] = true;
              });
                    ...
                    <Table size="medium">
                      <TableHead>
                        <TableRow sx={{ '& th': { borderBottom: '2px solid' + themeColors.border, py: 1.2 } }}>
                          ...
                          <TableCell sx={{ fontWeight: 900, color: themeColors.textSecondary, fontSize: '0.78rem' }}>
                            BASE RATE (INR)
                          </TableCell>
                          <TableCell sx={{ fontWeight: 900, color: themeColors.textSecondary, fontSize: '0.78rem' }} align="center">
                            VIP PICK
                          </TableCell>
                          <TableCell sx={{ fontWeight: 900, color: themeColors.textSecondary, fontSize: '0.78rem' }} align="center">
                            STATUS
                          </TableCell>
                          <TableCell sx={{ fontWeight: 900, color: themeColors.textSecondary, fontSize: '0.78rem' }} align="right">
                            ACTIONS
                          </TableCell>
                        </TableRow>
                      </TableHead>
                            ...
                            <TableCell>
                              ...
                              <Box sx={{ display: 'flex', gap: 1, mt: 0.5 }}>
                                <Chip
                                  label="Trained"
                                  size="small"
                                  icon={<Shield sx={{ fontSize: '10px !important' }} />}
          ...
          {selectedPackage && (
            <Box>
              ...
              <Typography sx={{ color: themeColors.textSecondary, mb: 3, lineHeight: 1.6, fontSize: '0.9rem' }}>
                {selectedPackage.description || 'Professional security and crowd control services tailored for your grand event.'}
              </Typography>
      ...
      >
        <Alert severity={notification.severity} variant="filled" sx={{ borderRadius: '18px', fontWeight: 700, fontSize: '0.82rem' }}>
