...
const InvitationCoupon = () => {
    ...
    const fetchCoupons = async (pageNum = 1, searchQuery = "") => {
        setLoading(true);
        try {
            const url = `${API_BASE_URL}/invitation-coupons?page=${pageNum}&limit=10&search=${encodeURIComponent(searchQuery)}`;
    ...
    const handleSave = async () => {
        setLoading(true);
        try {
            const response = await fetch(`${API_BASE_URL}/invitation-coupons`, {
                method: "POST",
    ...
    return (
        <Box p={3}>
            <Typography variant="h4" fontWeight="bold" gutterBottom>Invitation & Printing Coupons</Typography>
