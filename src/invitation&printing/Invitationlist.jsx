...
const Invitationlist = () => {
  ...
  const handleView = (pkg) => { setSelectedInvitation(pkg.rawPackage); setOpenViewDialog(true); };
  ...
  return (
    <Box sx={{ p: 4, minHeight: '100vh', bgcolor: themeColors.background, color: themeColors.textMain }}>
      ...
      <Box sx={{ display: 'flex', gap: 2.5, mb: 4, flexWrap: 'wrap' }}>
        <HeaderStat label="Card Designs" value={stats.total} icon={<Description />} color={themeColors.accent} gradient={themeColors.gradientPrimary} />
        ...
        <HeaderStat label="Print Shops" value={stats.vendors} icon={<Storefront />} color={themeColors.primary} />
      </Box>
      ...
      <Paper elevation={0} sx={{ p: 1.8, mb: 3.5, borderRadius: '18px', border: '1px solid', borderColor: themeColors.border, display: 'flex', alignItems: 'center', justifyContent: 'space-between', bgcolor: 'white' }}>
        <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
          <TextField
            placeholder="Search printers or styles..."
            size="small"
            value={searchTerm}
        ...
        >
          {Object.values(expandedVendors).some(v => v) ? 'Collapse Studios' : 'View All Designers'}
        </Button>
      </Paper>
            ...
            <Paper key={vendor.providerId} elevation={0} sx={{
              ...
              <Box onClick={() => toggleVendor(vendor.providerId)} sx={{
                ...
                <Box sx={{ textAlign: 'right', px: 1.5 }}>
                  <Typography sx={{ fontWeight: 900, color: themeColors.textSecondary, fontSize: '0.75rem' }}>{vendor.packages.length} DESIGNS</Typography>
                    ...
                    <Table size="medium">
                      <TableHead>
                        <TableRow sx={{ '& th': { borderBottom: '2px solid' + themeColors.border, py: 1.2 } }}>
                          <TableCell sx={{ fontWeight: 900, color: themeColors.textSecondary, fontSize: '0.78rem' }}>DESIGN NAME</TableCell>
                          ...
                          <TableRow key={pkg._id} sx={{ '& td': { py: 1.6 }, '&:hover': { bgcolor: '#F9FAFB' } }}>
                            <TableCell>
                              ...
                              <Box sx={{ display: 'flex', gap: 1, mt: 0.5 }}>
                                <Chip label="Print Art" size="small" icon={<Print sx={{ fontSize: '10px !important' }} />} sx={{ height: 18, fontSize: '0.6rem', fontWeight: 700, bgcolor: '#f0fdf4', color: '#16a34a' }} />
                              </Box>
                            </TableCell>
                            <TableCell>
      ...
      <Dialog open={openViewDialog} onClose={() => setOpenViewDialog(false)} maxWidth="sm" fullWidth PaperProps={{ sx: { borderRadius: '24px' } }}>
        ...
        <DialogContent sx={{ pb: 4 }}>
          {selectedInvitation && (
            <Box>
              <Typography variant="h5" sx={{ fontWeight: 900, mb: 1.5, color: themeColors.accent }}>{selectedInvitation.packageName}</Typography>
              <Typography sx={{ color: themeColors.textSecondary, mb: 3, lineHeight: 1.6, fontSize: '0.9rem' }}>{selectedInvitation.description}</Typography>
              ...
              <Grid container spacing={2}>
                <Grid item xs={6}>
                  <Paper variant="outlined" sx={{ p: 2, borderRadius: '16px', bgcolor: '#F9FAFB' }}>
                    ...
                    <Typography variant="h5" sx={{ fontWeight: 900, color: themeColors.success }}>₹{selectedInvitation.packagePrice.toLocaleString()}</Typography>
                  </Paper>
                </Grid>
                <Grid item xs={6}>
                  <Paper variant="outlined" sx={{ p: 2, borderRadius: '16px', bgcolor: '#F9FAFB' }}>
                    ...
                    <Typography variant="h6" sx={{ fontWeight: 900, color: themeColors.primary }}>
                      ₹{selectedInvitation.advanceBookingAmount.toLocaleString()}
                    </Typography>
                  </Paper>
                </Grid>
              </Grid>
              ...
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                {(selectedInvitation.services || []).map((s, i) => (
