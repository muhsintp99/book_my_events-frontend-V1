...
export default function CategoryManagement() {
  ...
  const fetchModules = useCallback(async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/secondary-modules`, {
        headers: {
      ...
      // Normalize IDs
      const normalizedModules = modulesList.map(m => ({
        ...m,
        _id: m._id?.$oid || m._id
      }));
      ...
      // Find Mehandi/Mehndi module as priority for this section
   // Find Invitation & Printing module as default
const invitationModule = normalizedModules.find(m =>
  (m.title || m.name || '').toLowerCase().includes('invitation') &&
  (m.title || m.name || '').toLowerCase().includes('printing')
);

let defaultModuleId = '';

if (invitationModule) {
  defaultModuleId = invitationModule._id;
} else if (normalizedModules.length > 0) {
  defaultModuleId = normalizedModules[0]._id;
}
      if (defaultModuleId) {
        setFormData(prev => ({ ...prev, module: defaultModuleId }));
        setSelectedModuleFilter(defaultModuleId);
      } else {
        setModulesError('No secondary modules available. Please configure secondary modules first.');
      }
    } catch (error) {
  ...
  const handleReset = () => {
   const invitationModule = modules.find(m =>
  (m.title || m.name || '').toLowerCase().includes('invitation') &&
  (m.title || m.name || '').toLowerCase().includes('printing')
);

const defaultModuleId =
  invitationModule?._id || (modules.length > 0 ? modules[0]._id : '');
  ...
  const handleEditClick = async (id) => {
    ...
    try {
      ...
      setEditFormData({
        ...
        parentCategory: (cat.parentCategory?._id?.$oid || cat.parentCategory?._id || cat.parentCategory || ''),
        module: (cat.module?._id?.$oid || cat.module?._id || cat.module || ''),
  ...
  const parentCats = filteredCategories.filter(c => !c.parentCategory);
  const subCats = filteredCategories.filter(c => c.parentCategory);
          ...
          <Stack spacing={0.5}>
            ...
            {category.parentCategory && (
              <Typography variant="caption" sx={{ color: '#ed6c02', fontStyle: 'italic' }}>
                Parent: {(() => {
                  const parent = categories.find(p => p.id === category.parentCategory);
                  return parent ? parent.names[currentLang] : 'Unknown';
                })()}
              </Typography>
            )}
          </Stack>
                        ...
                        <>
                          <TableRow sx={{ backgroundColor: '#f5f9ff' }}>
                            <TableCell colSpan={6} sx={{ py: 1.5 }}>
                              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                <Box sx={{ width: 4, height: 18, bgcolor: '#1976d2', borderRadius: 1 }} />
                                <Typography variant="subtitle2" sx={{ fontWeight: 700, color: '#1976d2', letterSpacing: 0.5 }}>
                                  PARENT CATEGORIES ({parentCats.length})
                                </Typography>
                              </Box>
                            </TableCell>
                          </TableRow>
                          {parentCats.map((category, index) => renderCategoryRow(category, index))}
                        </>
                      )}
                      ...
                      {subCats.length > 0 && (
                        <>
                          <TableRow sx={{ backgroundColor: '#fff7ed' }}>
                            <TableCell colSpan={6} sx={{ py: 1.5, borderTop: '2px solid #fff' }}>
                              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                <Box sx={{ width: 4, height: 18, bgcolor: '#ed6c02', borderRadius: 1 }} />
                                <Typography variant="subtitle2" sx={{ fontWeight: 700, color: '#ed6c02', letterSpacing: 0.5 }}>
                                  SUBCATEGORIES ({subCats.length})
                                </Typography>
                              </Box>
                            </TableCell>
                          </TableRow>
                          {subCats.map((category, index) => renderCategoryRow(category, index))}
                        </>
                      )}
                    </>
                  ) : (
