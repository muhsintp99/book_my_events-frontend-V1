...
export default function CategoryManagement() {
  ...
  const fetchModules = useCallback(async () => {
    ...
    try {
      ...
      // Find Emcee module as default
      const emceeModule = normalizedModules.find(m =>
        (m.title || m.name || '').toLowerCase().includes('emcee')
      );

      let defaultModuleId = '';

      if (emceeModule) {
        defaultModuleId = emceeModule._id;
  ...
  const handleReset = () => {
    const emceeModule = modules.find(m =>
      (m.title || m.name || '').toLowerCase().includes('emcee')
    );

    const defaultModuleId =
      emceeModule?._id || (modules.length > 0 ? modules[0]._id : '');
