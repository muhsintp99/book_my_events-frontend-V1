...
function AddMehandi() {
  ...
  const fetchModules = async () => {
    try {
      ...
      if (activeModuleId && filtered.length) {
        ...
        const mehandiMod = combined.find(
          (m) => (m.title || '').toLowerCase().includes('mehandi') || (m.title || '').toLowerCase().includes('mehndi')
        );
        if (mehandiMod) {
          setFormData((prev) => ({ ...prev, module: mehandiMod._id }));
          setModules([mehandiMod]);
        }
      }
    } catch (err) {
  ...
  const handleReset = () => {
    // Find Mehandi module ID from allModules
    const mehandiMod = (allModules || []).find(
      (m) => (m.title || '').toLowerCase().includes('mehandi') || (m.title || '').toLowerCase().includes('mehndi')
    );
    ...
    setFormData({
      ...
      maximumDeliveryTime: '',
      zone: '',
      module: mehandiMod ? mehandiMod._id : activeModuleId || '',
      latitude: '',
      longitude: '',
      ownerFirstName: '',
      ownerLastName: '',
  ...
  return (
    <Box sx={{ p: 3, backgroundColor: '#f9f9f9', borderRadius: 2 }}>
      <Typography variant="h4" fontWeight="bold" sx={{ mb: 3 }}>
        {isEditMode ? 'Edit Mehandi Artist Provider' : 'Add Mehandi Artist Provider'}
      </Typography>
      ...
      <Box sx={{ mb: 3, p: 2, border: '1px solid #e0e0e0', borderRadius: 2, bgcolor: '#fff' }}>
        <Typography variant="h5" fontWeight="bold" sx={{ mb: 2 }}>
          Mehandi Specialisation & Services
        </Typography>
      ...
      <Box sx={{ mb: 3 }}>
        ...
        {/* Multi-Zone (supported for Makeup, Photography, Mehandi) */}
