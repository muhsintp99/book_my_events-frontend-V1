...
const MehandiCoupon = () => {
  ...
  const fetchCoupons = async (pageNum = 1, searchQuery = "") => {
    ...
    try {
      const url = `${API_BASE_URL}/mehandi-coupons?page=${pageNum}&limit=10&search=${encodeURIComponent(searchQuery)}`;
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`${API_BASE_URL}/mehandi-coupons`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
