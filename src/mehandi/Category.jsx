...
export default function CategoryManagement() {
  ...
  const fetchModules = useCallback(async () => {
    ...
    try {
      ...
      const mehandiModule = normalizedModules.find(m =>
        (m.title || m.name || '').toLowerCase().includes('mehandi') ||
        (m.title || m.name || '').toLowerCase().includes('mehndi')
      );

      let defaultModuleId = '';
      if (mehandiModule) {
        defaultModuleId = mehandiModule._id;
  ...
  const handleReset = () => {
    const mehandiModule = modules.find(m =>
      (m.title || m.name || '').toLowerCase().includes('mehandi') ||
      (m.title || m.name || '').toLowerCase().includes('mehndi')
    );
    const defaultModuleId = mehandiModule?._id || (modules.length > 0 ? modules[0]._id : '');
