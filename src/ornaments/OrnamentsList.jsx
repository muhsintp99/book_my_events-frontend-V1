...
import {
  ...
  Diamond, Inventory, EventAvailable
...
const OrnamentsList = () => {
  ...
  const handleView = (pkg) => { setSelectedOrnament(pkg.rawPackage); setOpenViewDialog(true); };
  ...
  return (
    <Box sx={{ p: 4, minHeight: '100vh', bgcolor: themeColors.background, color: themeColors.textMain }}>
      ...
      <Box sx={{ display: 'flex', gap: 2.5, mb: 4, flexWrap: 'wrap' }}>
        <HeaderStat label="Jewels" value={stats.total} icon={<Diamond />} color={themeColors.accent} gradient={themeColors.gradientPrimary} />
        ...
        <HeaderStat label="Jewellers" value={stats.vendors} icon={<Storefront />} color={themeColors.primary} />
      </Box>
      ...
      <Paper elevation={0} sx={{ p: 1.8, mb: 3.5, borderRadius: '18px', border: '1px solid', borderColor: themeColors.border, display: 'flex', alignItems: 'center', justifyContent: 'space-between', bgcolor: 'white' }}>
        <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
          <TextField
            placeholder="Search jewellers or gems..."
            size="small"
            value={searchTerm}
        ...
        >
          {Object.values(expandedVendors).some(v => v) ? 'Collapse Jewellers' : 'View All Gems'}
        </Button>
      </Paper>
            ...
            <Paper key={vendor.providerId} elevation={0} sx={{
              ...
              <Box onClick={() => toggleVendor(vendor.providerId)} sx={{
                ...
                <Box sx={{ textAlign: 'right', px: 1.5 }}>
                  <Typography sx={{ fontWeight: 900, color: themeColors.textSecondary, fontSize: '0.75rem' }}>{vendor.packages.length} GEMS</Typography>
                    ...
                    <Table size="medium">
                      <TableHead>
                        <TableRow sx={{ '& th': { borderBottom: '2px solid' + themeColors.border, py: 1.2 } }}>
                          <TableCell sx={{ fontWeight: 900, color: themeColors.textSecondary, fontSize: '0.78rem' }}>GEM INFO</TableCell>
                            ...
                            <TableCell>
                              ...
                              <Box sx={{ display: 'flex', gap: 1, mt: 0.5 }}>
                                <Chip 
                                  ...
                                  size="small" 
                                  icon={pkg.mode === 'rental' ? <EventAvailable sx={{ fontSize: '10px !important' }} /> : <Inventory sx={{ fontSize: '10px !important' }} />} 
                                  sx={{ height: 18, fontSize: '0.6rem', fontWeight: 700, bgcolor: pkg.mode === 'rental' ? '#f0f9ff' : '#fef2f2', color: pkg.mode === 'rental' ? '#0369a1' : '#dc2626' }} 
                                />
                              </Box>
                            </TableCell>
                            <TableCell>
                              <Typography sx={{ fontWeight: 900, color: themeColors.success, fontSize: '1.05rem' }}>₹{pkg.price.toLocaleString()}</Typography>
                              <Typography variant="caption" sx={{ color: themeColors.textSecondary, fontSize: '0.65rem' }}>{pkg.mode === 'rental' ? 'per event' : 'full ownership'}</Typography>
                            </TableCell>
      ...
      <Dialog open={openViewDialog} onClose={() => setOpenViewDialog(false)} maxWidth="sm" fullWidth PaperProps={{ sx: { borderRadius: '24px' } }}>
        <DialogTitle sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', pt: 3 }}>
          <Typography sx={{ fontWeight: 900, fontSize: '1.2rem' }}>Gem Details</Typography>
          <IconButton onClick={() => setOpenViewDialog(false)}><Close /></IconButton>
        </DialogTitle>
        <DialogContent sx={{ pb: 4 }}>
          {selectedOrnament && (
            <Box>
              <Typography variant="h5" sx={{ fontWeight: 900, mb: 1.5, color: themeColors.accent }}>{selectedOrnament.name}</Typography>
              <Typography sx={{ color: themeColors.textSecondary, mb: 3, lineHeight: 1.6, fontSize: '0.9rem' }}>{selectedOrnament.description}</Typography>
                ...
                <Grid item xs={6}>
                  <Paper variant="outlined" sx={{ p: 2, borderRadius: '16px', bgcolor: '#F9FAFB' }}>
                    ...
                    <Typography variant="h5" sx={{ fontWeight: 900, color: themeColors.success }}>
                      ₹{selectedOrnament.availabilityMode === 'rental' ? selectedOrnament.rentalPricing?.totalPrice?.toLocaleString() : selectedOrnament.buyPricing?.totalPrice?.toLocaleString()}
                    </Typography>
                  </Paper>
                </Grid>
                <Grid item xs={6}>
                  <Paper variant="outlined" sx={{ p: 2, borderRadius: '16px', bgcolor: '#F9FAFB' }}>
                    ...
                    <Typography variant="h6" sx={{ fontWeight: 900, color: themeColors.primary, textTransform: 'uppercase' }}>
                      {selectedOrnament.availabilityMode}
                    </Typography>
                  </Paper>
                </Grid>
              </Grid>
              ...
              <Typography variant="subtitle2" sx={{ fontWeight: 900, mb: 2 }}>Specifications & Suitable For:</Typography>
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                {selectedOrnament.weight > 0 && <Chip label={`${selectedOrnament.weight}g weight`} size="small" sx={{ fontWeight: 700 }} />}
                {(selectedOrnament.features?.suitableFor || []).map((s, i) => (
                  <Chip key={i} label={s} size="small" sx={{ fontWeight: 700 }} />
                ))}
              </Box>
            </Box>
          )}
