...
function AddLight() {
  ...
  const fetchModules = async () => {
    try {
      ...
      if (activeModuleId && filtered.length) {
        ...
        const lightMod = combined.find(
          (m) => (m.title || '').toLowerCase().includes('light') || (m.title || '').toLowerCase().includes('sound')
        );
        if (lightMod) {
          setFormData((prev) => ({ ...prev, module: lightMod._id }));
          setModules([lightMod]);
        }
      }
    } catch (err) {
  ...
  const handleReset = () => {
    // Find Light & Sound module ID from allModules
    const lightMod = (allModules || []).find(
      (m) => (m.title || '').toLowerCase().includes('light') || (m.title || '').toLowerCase().includes('sound')
    );
    ...
    setFormData({
      ...
      maximumDeliveryTime: '',
      zone: '',
      module: lightMod ? lightMod._id : activeModuleId || '',
      latitude: '',
      longitude: '',
      ownerFirstName: '',
      ownerLastName: '',
  ...
  return (
    <Box sx={{ p: 3, backgroundColor: '#f9f9f9', borderRadius: 2 }}>
      <Typography variant="h4" fontWeight="bold" sx={{ mb: 3 }}>
        {isEditMode ? 'Edit Light & Sounds Provider' : 'Add Light & Sounds Provider'}
      </Typography>
      ...
      <Box sx={{ mb: 3, p: 2, border: '1px solid #e0e0e0', borderRadius: 2, bgcolor: '#fff' }}>
        <Typography variant="h5" fontWeight="bold" sx={{ mb: 2 }}>
          Light & Sound Specialisation & Services
        </Typography>
