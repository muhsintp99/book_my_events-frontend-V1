...
const LightList = () => {
    ...
    const fetchItems = useCallback(async () => {
        try {
            ...
            // setNotification({ open: true, message: 'Failed to fetch items', severity: 'error' });
        } finally {
            setLoading(false);
        }
    }, [API_URL]);
    ...
    return (
        <Box p={3}>
            <Stack direction="row" justifyContent="space-between" alignItems="center" mb={3}>
                <Typography variant="h4" fontWeight="bold">Light & Sound List</Typography>
                <Button variant="contained" color="primary" startIcon={<AddIcon />} onClick={() => navigate('/lights/AddProvider')}>
                    Add Light & Sound
                </Button>
            </Stack>
                ...
                <Table>
                    ...
                    <TableBody>
                        {loading ? (
                            ...
                            <TableRow>
                                <TableCell colSpan={7} align="center" py={5}>No items found</TableCell>
                            </TableRow>
                        ) : (
                            filtered.map((item, idx) => (
                                <TableRow key={item._id} hover>
                                    ...
                                    <TableCell align="right">
                                        <IconButton size="small" color="primary" onClick={() => navigate(`/lights/AddProvider/${item._id}`)}><Edit /></IconButton>
            ...
            <Dialog open={openDeleteDialog} onClose={() => setOpenDeleteDialog(false)}>
                ...
                <DialogContent><DialogContentText>Are you sure you want to delete this item?</DialogContentText></DialogContent>
                <DialogActions>
