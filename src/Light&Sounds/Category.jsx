...
export default function CategoryManagement() {
  ...
  const fetchModules = useCallback(async () => {
    ...
    try {
      ...
      // Find Light & Sound module as default
      const lightModule = normalizedModules.find(m =>
        (m.title || m.name || '').toLowerCase().includes('light') ||
        (m.title || m.name || '').toLowerCase().includes('sound')
      );

      let defaultModuleId = '';

      if (lightModule) {
        defaultModuleId = lightModule._id;
  ...
  const handleReset = () => {
    const lightModule = modules.find(m =>
      (m.title || m.name || '').toLowerCase().includes('light') ||
      (m.title || m.name || '').toLowerCase().includes('sound')
    );

    const defaultModuleId =
      lightModule?._id || (modules.length > 0 ? modules[0]._id : '');
