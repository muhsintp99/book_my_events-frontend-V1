...
function AddPanthal() {
  ...
  const fetchModules = async () => {
    try {
      ...
      if (activeModuleId && filtered.length) {
        ...
        const panthalMod = combined.find(
          (m) => (m.title || '').toLowerCase().includes('panthal') || (m.title || '').toLowerCase().includes('decoration')
        );
        if (panthalMod) {
          setFormData((prev) => ({ ...prev, module: panthalMod._id }));
          setModules([panthalMod]);
        }
      }
    } catch (err) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEditMode) {
        ...
        setTimeout(() => {
          navigate('/panthal/panthalprovider');
        }, 2000);
        return;
      }
  ...
  const handleReset = () => {
    // Find Panthal module ID from allModules
    const panthalMod = (allModules || []).find(
      (m) => (m.title || '').toLowerCase().includes('panthal') || (m.title || '').toLowerCase().includes('decoration')
    );
    ...
    setFormData({
      ...
      maximumDeliveryTime: '',
      zone: '',
      module: panthalMod ? panthalMod._id : activeModuleId || '',
      latitude: '',
      longitude: '',
      ownerFirstName: '',
      ownerLastName: '',
  ...
  return (
    <Box sx={{ p: 3, backgroundColor: '#f9f9f9', borderRadius: 2 }}>
      <Typography variant="h4" fontWeight="bold" sx={{ mb: 3 }}>
        {isEditMode ? 'Edit Panthal & Decorations Provider' : 'Add Panthal & Decorations Provider'}
      </Typography>
      ...
      <Box sx={{ mb: 3, p: 2, border: '1px solid #e0e0e0', borderRadius: 2, bgcolor: '#fff' }}>
        <Typography variant="h5" fontWeight="bold" sx={{ mb: 2 }}>
          Panthal Specialisation & Services
        </Typography>
