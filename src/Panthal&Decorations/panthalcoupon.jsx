...
const PanthalCoupon = () => {
    ...
    const fetchCoupons = async (searchQuery = "") => {
        setLoading(true);
        try {
            const url = `${API_BASE_URL}/panthal-coupons?search=${encodeURIComponent(searchQuery)}`;
    ...
    const handleSave = async () => {
        setLoading(true);
        try {
            const response = await fetch(`${API_BASE_URL}/panthal-coupons`, {
                method: "POST",
    ...
    return (
        <Box p={3}>
            <Typography variant="h4" fontWeight="bold" gutterBottom>Panthal & Decorations Coupons</Typography>
