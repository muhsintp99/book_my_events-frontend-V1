...
export default function CategoryManagement() {
  ...
  const fetchModules = useCallback(async () => {
    ...
    try {
      ...
      // Find Panthal module as default
      const panthalModule = normalizedModules.find(m =>
        (m.title || m.name || '').toLowerCase().includes('panthal') ||
        (m.title || m.name || '').toLowerCase().includes('decoration')
      );

      let defaultModuleId = '';

      if (panthalModule) {
        defaultModuleId = panthalModule._id;
  ...
  const handleReset = () => {
    const panthalModule = modules.find(m =>
      (m.title || m.name || '').toLowerCase().includes('panthal') ||
      (m.title || m.name || '').toLowerCase().includes('decoration')
    );

    const defaultModuleId =
      panthalModule?._id || (modules.length > 0 ? modules[0]._id : '');
