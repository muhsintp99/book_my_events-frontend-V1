...
/* ============ Premium Red Luxe Theme (Matches Makeup) ============ */
const themeColors = {
  primary: '#2D3436',
  accent: '#E15B64',
  accentLight: '#FFF8F9',
...
const PhotoList = () => {
    ...
    try {
      ...
      if (data?.data && Array.isArray(data.data)) {
        const mapped = data.data.map((p, idx) => ({
          _id: p._id, id: idx + 1,
          title: p.packageTitle || 'Untitled',
          description: p.description || '',
          price: p.price ?? 0,
          travelToVenue: p.travelToVenue ?? false,
          providerId: p.provider?._id || p.provider?.id || 'unknown',
          providerName: p.provider?.firstName && p.provider?.lastName ? `${p.provider.firstName} ${p.provider.lastName}` : (p.provider?.firstName || p.provider?.name || '—'),
          providerEmail: p.provider?.email || '', providerPhone: p.provider?.phone || '',
          isTopPick: p.isTopPick ?? false, isActive: p.isActive ?? false,
          rawPackage: p,
        }));
  ...
  const handleView = (pkg) => { setSelectedPhoto(pkg.rawPackage); setOpenViewDialog(true); };
  ...
  return (
    <Box sx={{ p: 4, minHeight: '100vh', bgcolor: themeColors.background, color: themeColors.textMain }}>
      ...
      <Box sx={{ display: 'flex', gap: 2.5, mb: 4, flexWrap: 'wrap' }}>
        <HeaderStat label="Portfolios" value={stats.total} icon={<ViewList />} color={themeColors.accent} gradient={themeColors.gradientPrimary} />
        ...
        <HeaderStat label="Studio Pros" value={stats.vendors} icon={<Storefront />} color={themeColors.primary} />
      </Box>
      ...
      <Paper elevation={0} sx={{ p: 1.8, mb: 3.5, borderRadius: '18px', border: '1px solid', borderColor: themeColors.border, display: 'flex', alignItems: 'center', justifyContent: 'space-between', bgcolor: 'white' }}>
        <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
          <TextField
            placeholder="Search studios or collections..."
            size="small"
            value={searchTerm}
        ...
        >
          {Object.values(expandedVendors).some(v => v) ? 'Collapse Studios' : 'View All Portfolios'}
        </Button>
      </Paper>
            ...
            <Paper key={vendor.providerId} elevation={0} sx={{
              ...
              <Box onClick={() => toggleVendor(vendor.providerId)} sx={{
                ...
                <Box sx={{ textAlign: 'right', px: 1.5 }}>
                  <Typography sx={{ fontWeight: 900, color: themeColors.textSecondary, fontSize: '0.75rem' }}>{vendor.packages.length} PORTFOLIOS</Typography>
                    ...
                    <Table size="medium">
                      <TableHead>
                        <TableRow sx={{ '& th': { borderBottom: '2px solid' + themeColors.border, py: 1.2 } }}>
                          <TableCell sx={{ fontWeight: 900, color: themeColors.textSecondary, fontSize: '0.78rem' }}>COLLECTION INFO</TableCell>
                          ...
                          <TableRow key={pkg._id} sx={{ '& td': { py: 1.6 }, '&:hover': { bgcolor: '#F9FAFB' } }}>
                            <TableCell>
                              ...
                              <Box sx={{ display: 'flex', gap: 1, mt: 0.5 }}>
                                {pkg.travelToVenue && <Chip label="Location Coverage" size="small" icon={<FlightTakeoff sx={{ fontSize: '10px !important' }} />} sx={{ height: 18, fontSize: '0.6rem', fontWeight: 700, bgcolor: '#eff6ff', color: '#2563eb' }} />}
                              </Box>
                            </TableCell>
                            <TableCell>
      ...
      <Dialog open={openViewDialog} onClose={() => setOpenViewDialog(false)} maxWidth="sm" fullWidth PaperProps={{ sx: { borderRadius: '24px' } }}>
        ...
        <DialogContent sx={{ pb: 4 }}>
          {selectedPhoto && (
            <Box>
              <Typography variant="h5" sx={{ fontWeight: 900, mb: 1.5, color: themeColors.accent }}>{selectedPhoto.packageTitle}</Typography>
              <Typography sx={{ color: themeColors.textSecondary, mb: 3, lineHeight: 1.6, fontSize: '0.9rem' }}>{selectedPhoto.description}</Typography>
              ...
              <Grid container spacing={2}>
                <Grid item xs={6}>
                  <Paper variant="outlined" sx={{ p: 2, borderRadius: '16px', bgcolor: '#F9FAFB' }}>
                    ...
                    <Typography variant="h5" sx={{ fontWeight: 900, color: themeColors.success }}>₹{selectedPhoto.price.toLocaleString()}</Typography>
                  </Paper>
                </Grid>
                <Grid item xs={6}>
                  <Paper variant="outlined" sx={{ p: 2, borderRadius: '16px', bgcolor: '#F9FAFB' }}>
                    <Typography variant="caption" sx={{ fontWeight: 800, color: themeColors.textSecondary }}>LOCATION</Typography>
                    <Typography variant="h6" sx={{ fontWeight: 900, color: selectedPhoto.travelToVenue ? themeColors.success : themeColors.textSecondary }}>
                      {selectedPhoto.travelToVenue ? 'TRAVEL INCLUDED' : 'STUDIO ONLY'}
                    </Typography>
                  </Paper>
                </Grid>
              </Grid>
              ...
              <Typography variant="subtitle2" sx={{ fontWeight: 900, mb: 2 }}>Included Services:</Typography>
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                {(selectedPhoto.includedServices || []).map((s, i) => (
                  <Chip key={i} label={s.title} size="small" sx={{ fontWeight: 700 }} />
                ))}
              </Box>
            </Box>
          )}
