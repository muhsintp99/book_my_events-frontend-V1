...
const FloristList = () => {
    ...
    const filtered = florists.filter(f =>
        ...
        (f.provider?.firstName || '').toLowerCase().includes(searchTerm.toLowerCase())
    );

    return (
        <Box p={3}>
            <Stack direction="row" justifyContent="space-between" alignItems="center" mb={3}>
                <Typography variant="h4" fontWeight="bold">Florist & Stage List</Typography>
                <Button variant="contained" color="primary" startIcon={<AddIcon />} onClick={() => navigate('/florist/AddProvider')}>
                    Add Florist
                </Button>
            </Stack>
                ...
                <Table>
                    ...
                    <TableBody>
                        {loading ? (
                            ...
                            <TableRow>
                                <TableCell colSpan={7} align="center" py={5}>No florists found</TableCell>
                            </TableRow>
                        ) : (
                            filtered.map((item, idx) => (
                                <TableRow key={item._id} hover>
                                    ...
                                    <TableCell align="right">
                                        <IconButton size="small" color="primary" onClick={() => navigate(`/florist/AddProvider/${item._id}`)}><Edit /></IconButton>
            ...
            <Dialog open={openDeleteDialog} onClose={() => setOpenDeleteDialog(false)}>
                ...
                <DialogContent><DialogContentText>Are you sure you want to delete this florist?</DialogContentText></DialogContent>
                <DialogActions>
