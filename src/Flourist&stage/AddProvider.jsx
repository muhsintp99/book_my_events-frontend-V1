...
function AddFlorist() {
  ...
  const fetchModules = async () => {
    try {
      ...
      if (activeModuleId && filtered.length) {
        ...
        // Fallback: try to find Florist module by name
        const floristMod = combined.find(
          (m) => (m.title || '').toLowerCase().includes('florist') || (m.title || '').toLowerCase().includes('flower')
        );
        if (floristMod) {
          setFormData((prev) => ({ ...prev, module: floristMod._id }));
          setModules([floristMod]);
        }
      }
    } catch (err) {
  ...
  const handleReset = () => {
    // Find Florist module ID from allModules
    const floristMod = (allModules || []).find(
      (m) => (m.title || '').toLowerCase().includes('florist') || (m.title || '').toLowerCase().includes('flower')
    );
    ...
    setFormData({
      ...
      maximumDeliveryTime: '',
      zone: '',
      module: floristMod ? floristMod._id : activeModuleId || '',
      latitude: '',
      longitude: '',
      ownerFirstName: '',
      ownerLastName: '',
  ...
  return (
    <Box sx={{ p: 3, backgroundColor: '#f9f9f9', borderRadius: 2 }}>
      <Typography variant="h4" fontWeight="bold" sx={{ mb: 3 }}>
        {isEditMode ? 'Edit Florist & Stage Provider' : 'Add Florist & Stage Provider'}
      </Typography>
      ...
      <Box sx={{ mb: 3, p: 2, border: '1px solid #e0e0e0', borderRadius: 2, bgcolor: '#fff' }}>
        <Typography variant="h5" fontWeight="bold" sx={{ mb: 2 }}>
          Florist & Stage Specialisation & Services
        </Typography>
