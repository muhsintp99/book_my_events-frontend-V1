...
export default function CategoryManagement() {
  ...
  const fetchModules = useCallback(async () => {
    ...
    try {
      ...
      // Find Florist module as default
      const floristModule = normalizedModules.find(m =>
        (m.title || m.name || '').toLowerCase().includes('florist') ||
        (m.title || m.name || '').toLowerCase().includes('flower')
      );

      let defaultModuleId = '';

      if (floristModule) {
        defaultModuleId = floristModule._id;
  ...
  const handleReset = () => {
    const floristModule = modules.find(m =>
      (m.title || m.name || '').toLowerCase().includes('florist') ||
      (m.title || m.name || '').toLowerCase().includes('flower')
    );

    const defaultModuleId =
      floristModule?._id || (modules.length > 0 ? modules[0]._id : '');
